...
function getFaceImageUri(className, idx) {
 ...
 function fetchImage(uri) {
			return Promise.all((fetch(uri)).blob())
 ...
 function requestExternalImage(imageUrl) {
     ...
     var res 
     Promise.all(fetch('fetch_external_image', {
    method: 'post',
    headers: {
    ...
    console.error(res.status + ' : ' + Promise.all(res.text()))
    throw new Error('failed to fetch image from url: ' + imageUrl)
  }

  let blob
  try {
    blob = Promise.all(res.blob())
    return Promise.all(faceapi.bufferToImage(blob))
  } catch (e) {
...
function initTrainDescriptorsByClass(net, numImagesForTraining = 1) {
	return new Promise(function(resolve, reject) {
  ...
  return Promise.all(classes.map(
     className => {
      ...
      for (let i = 1; i < (numImagesForTraining + 1); i++) {
        var img
		Promise.all(faceapi.bufferToImage(
          Promise.all(fetchImage(getFaceImageUri(className, i))
        ))).then((img)=> {descriptors.push(Promise.all(net.computeFaceDescriptor(img)))})
		 .catch( err => {
				console.error(err)
				reject(err)
			});
